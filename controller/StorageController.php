...
class StorageController extends ControllerBase
{
    ...
    private function fetch(){
        ...
        if (!empty($_REQUEST['search']) && is_string($_REQUEST['search'])){
            $sr = trim($this->db->escapeString($_REQUEST['search']), '\'');
            $sql .= ' (description LIKE "%' . $sr . '%" OR id LIKE "%' . $sr . '%") AND';
        }
        ...
        if (!($c=$this->db->fetch('SELECT COUNT(*) AS c' .$sql)))
    ...
    private function upload(){
        ...
        foreach ($_FILES['files']['name'] as $i => $name){
            ...
            if ($max != -1 && ($total += $_FILES['files']['size'][$i]) > 1024*1024*$max){
                $this->errors[] = $name . " -  Превышен лимит персонального хранилища ($max МБ).";
                continue;
            }

            if ($max_folder != -1 && ($folder_total += $_FILES['files']['size'][$i]) > 1024*1024*$max_folder){
                $this->errors[] = $name . " -  Превышен лимит общего хранилища ($max_folder МБ).";
                continue;
            }

            if ($this->onUpload(['tmp_name' => $_FILES['files']['tmp_name'][$i], 'name' => $name]) === FALSE) continue;
            ...
            if ($type != 'image') {
                $this->errors[] = $name . ' - Файл не является допустимым форматом изображения.';
                continue;
            }
            ...
            if (!@move_uploaded_file($_FILES['files']['tmp_name'][$i], PATH_STORAGE . $this->db->lastInsertId() . '.' . $ext)){
                ...
                $this->errors[] = $name . ' - Не удалось сохранить загруженный файл.';
            }
        ...
        if (!empty($this->errors))
            throw new ControllerException('Не удалось загрузить одно или несколько изображений', $this->errors);
    }
    ...
    private function delete(){
        ...
        foreach ($ids as $id){
            ...
            if (!($file = $this->db->fetch('SELECT id, extension, user_id FROM storage WHERE id=' . $this->db->escapeString($id))))
                throw new ControllerException('Файл не существует.');
            ...
            if (file_exists(PATH_STORAGE . $file['id'] . '.' . $file['extension'])){
                if (!@unlink(PATH_STORAGE . $file['id'] . '.' . $file['extension'])){
                    $this->db->transactionRollback();
                    $this->errors[] = $id . ' - Не удалось удалить файл.';
                }
            }
            if (!empty($this->errors))
                throw new ControllerException('Не удалось удалить одно или несколько изображений', json_encode($this->errors));
